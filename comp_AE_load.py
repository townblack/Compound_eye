...
class AE(object):
    ...
    def train(self, config):
        ...
        trainimg = trainimg[:2750]
        trainlabel = np.load(cwd + '/PASCAL_VOC_2012/0.2/compoundData_seg_train.npy').astype(float)
        ...
        for epoch in range(config.epoch):
            ...
            self.display(epoch, config.epoch, ds_trainimg, ds_trainlabel,  didx, config.checkpoint_dir)



    # def display(self, _epoch, total_epochs, _trainimg, _trainlabel, _testimg, _testlabel, _didx, save_dir):
    def display(self, _epoch, total_epochs, _trainimg, _trainlabel, _didx, save_dir):
