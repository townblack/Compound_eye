...
for k in [1,3,5,7,9,11,13,15]:
    ...
    cwd = os.getcwd()
    compoundData_seg = h5py.File(cwd+"/coco/10/seg_%s.mat" %(name))
    compoundData_seg.keys()

    # Save directories
    knndir = cwd+'/coco/%d/train/knn_%s/%d' % (eye_size, name,k)
    ...
    # knn
    knn = np.load(os.getcwd()+"/NN/NN%d_441_knn.npy" %(k))
    ...
    for i in range(len(compoundData_gt)-1000):
