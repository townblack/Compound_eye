...
# Save directories
datadir = cwd+'/PASCAL_VOC_2012/%d/%s_pre_dot' %(eye_size, name)
...
compound_mean = np.reshape(compoundData, (-1,3))
compound_mean = np.mean(compound_mean, axis=0)
compound_mean = np.tile(compound_mean, (441,10,10,1))
...
mean = np.load(cwd+'/PASCAL_VOC_2012/%d/r0/train_pre_dot/mean.npy' %(eye_size))

for i in range(len(compoundData)):
    np.save(datadir+'/%05d' % (i), compoundData[i]-mean)
